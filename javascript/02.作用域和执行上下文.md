...
# 变量提升(let和const)
所谓的变量提升，可以理解为变量是已经存在于当前作用域的变量对象中。
...
***实际上，let声明的变量在变量对象初始化时的，会被创建，但是并不会被初始化，也就是没有值。所以在let声明变量前去访问变量，实际上连undefined值都没有，所以会报出异常，也就是所谓的暂时性死区。暂时性死区就意味着变量在初始化之前，不能被使用***

***这一点特性在const中也存在***

## 变量提升本质
如上文所述，所谓的变量提升，可以理解为变量是已经存在于当前作用域的变量对象中。  
...
![](https://tva1.sinaimg.cn/large/00831rSTgy1gdjfcp07zpj31400ltq5o.jpg)
