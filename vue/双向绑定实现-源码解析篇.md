...
- 普通指令则调用指令处理合集(compileUtil)中的方法,获取对应指令的视图更新方法，生成观察者Wathcer(通过传入实例化Wathcher的回调函数中，闭包获取到需要更新的node，与node产生联系)
```javascript
function Compile(el) {
...
##观察者Watcher
Deps用于保存当前观察者依赖数据的订阅者中心（可能有多个，例如v-text="a&b",在实例化Watcher时会往a、b里的Dep都添加订阅,所以需要获取a、b的DepId防止重复添加订阅），防止重复添加watcher到依赖数据的订阅者中心中
